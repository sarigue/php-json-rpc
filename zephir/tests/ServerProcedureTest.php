...
class ServerProcedureTest extends \PHPUnit_Framework_TestCase
{
    ...
    {
        $server = new Server;
        $server->bind('getAllTasks', 'JsonRpcRambler\Tests\A', 'getNothing');
        $server->executeProcedure('getAllTasks');
    }
    ...
    {
        $server = new Server;
        $server->bind('getAllA', 'JsonRpcRambler\Tests\A', 'getAll');
        $server->bind('getAllB', 'JsonRpcRambler\Tests\B', 'getAll');
    ...
    /**
     * @expectedException \JsonRpcRambler\Exceptions\ResponseEncodingFailure
     */
